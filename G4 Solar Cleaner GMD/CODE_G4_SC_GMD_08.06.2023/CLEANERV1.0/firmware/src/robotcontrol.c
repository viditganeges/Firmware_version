...
static int pwm1ChangeValue             = DEFAULT_PWM_CHANGE_VALUE;
static int pwm2ChangeValue             = DEFAULT_PWM_CHANGE_VALUE;
static int currentPwm1Duty             = IDLE_PWM_DUTY;                        /* Declared as a float to enable fraction pwm Increment/decrement though its converted to integer in pwm.c */
static int currentPwm2Duty             = IDLE_PWM_DUTY;                        /* Declared as a float to enable fraction pwm Increment/decrement though its converted to integer in pwm.c */
...
static int32_t linearMotorStop_timecount = 0;      
...
static bool IsDirectionChange (eRobotDirection tarDirection, eRobotDirection curDirection)
{
  ...
  else if(curDirection == UNKNOWN)                                                       //changed
  {
...
//        brushtime = (getBrushPWMVal->brushTime)*1000;
        BrushMotorStopTimerOn(2000);    //changed to stop for 2SEC in positive end
      }
      else
      {
        brushtime = (getBrushPWMVal->brushTime)*1000;
...
{
  if(GetSlowAccelState() == 1) //ACCELINPROCESS
  {
  ...
  if(GetAccelState() == 1)  //ACCELINPROCESS
  {
    if(--pwm_ChangeTimeCount <= 0)
    {
      IncrementPwm();
  ...
  else if(GetDecelState() == 1) //DECCELINPROCESS
  {
    if(--pwm_ChangeTimeCount <= 0)
    {
      DecrementPwm();

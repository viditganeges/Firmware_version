...
//    ZigbeeTxBuffer[5] = (cmdid >> 16) & 0xFF;
...
bool zigbee_set_parameter(enum ZIGBEE_COMMAND_ID cmdid, uint8_t data)
{
    memset(ZigbeeTxBuffer, 0, sizeof(ZigbeeTxBuffer));
    ZigbeeTxBuffer[0] = ZIGBEE_FRAME_START_DELIMITER;
    ZigbeeTxBuffer[1] = 0x00;
    ...
    ZigbeeTxBuffer[5] = (cmdid >> 8) & 0xFF;
    ZigbeeTxBuffer[6] = cmdid & 0xFF;
    ZigbeeTxBuffer[7] = data;
//    memcpy(&ZigbeeTxBuffer[7], data, length);
    ZigbeeTxBuffer[8] = zigbee_checksum(&ZigbeeTxBuffer[3] , 5);
    ZigbeeTxSize = 9;;  
    USART0_ReadAbort();
    ZigbeeState = TRANSMITPACKET;
    ZigbeeControl();  
...
//void ZigbeeNetworkReset(uint8_t data)
void ZigbeeNetworkReset(void)
{
//    zigbee_set_parameter(NR0, data);
    zigbee_get_parameter(NR);
}
...
{
    int response = atoi((char *)data);
    if(response == 0)
    {
        ZIGBEE_RESET_SUCCESS = false;
    }

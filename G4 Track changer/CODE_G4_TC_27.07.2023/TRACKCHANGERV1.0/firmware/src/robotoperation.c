...
static void TC_Reverse_TimerOn (uint32_t rfid_sense_TimeCount_ms);
...
extern uint64_t GetLPTimerMsTicks (void);
...
//#endif    
  ...
  case TC_WAIT_FOR_PROXIMITY :
    ...
    GetLastReadRfidData(); 
    uint32_t RFID = atoi((char*)lastReadRfidData);
...
//        else if((GetOperationMode() == MANUAL)&&(specificrow > 0))
        else if((GetOperationMode() == MANUAL)&&(specificrow > 0)&& (C_ROW == 0))    
        {
            if(read_rfid == 1)
            {
               Rfid_Sense_TimerOn(20000);
...
//        ReportTrackChangerStatus(TC_REPORT_ROW_DONE,numberOfRows - rowsDone);
  ...
  case TC_CHECK_FOR_REMAINING_ROWS : 
      ...
      {
        if((rowsDone) >= getNorows->Row[Daycount - 1])    
        {
          SetTrackChangerState(TC_STOP);
          tcendtime();
        }
...
//            char data[30];
  ...
  case TC_REVERSE_AND_GOTO_START :
    ...
    SetTrackChangerState(TC_WAIT_FOR_EDGE);
    prevStatetime = GetLPTimerMsTicks();
    nextStateTimeout = MAX_TIME_FOR_ONE_PULSE_TRACK_CHANGER_PROX*tempSetTrackOpData.sumOfPulseCount;
...
//#endif
  ...
  case TC_JOG_AND_FIND_POSITION :
    {
      eJogStatus jogStatus = JogAndFindPosition((uint8_t*)&rowsDone);
...
//          }
  ...
  case CHECK_RFID_AND_ASSIGN_INDEX:
    ...
    readData = atoi((char*)lastReadRfidData);
...
eCleanerStatus OperateCleaner(uint16_t index)
{
    ...
    if(isCommTimeOver)
    {
      ...
      else
      {
        if(commRetryCount>=3)
        {
          ...
          SendCommandToCleaner(tempSetTrackOpData.cleanerMacAddress,STOP_CLEANER,(char*)"7,2");
  ...
  case C_GET_STATUS: 
    if(!isTimeOver)
    {
      if(!IsCleanerDone())
      {
        if(isCommTimeOver)
        {
          if((SendCommandToCleaner(tempSetTrackOpData.cleanerMacAddress,GET_CLEANER_STATUS,(char*)"2")==SUCCESS))
          {
            ClearIsCleanerDone();
          }
          else
  ...
  case C_STOP: 
    ...
    if(cStart)
    {
      SendCommandToCleaner(tempSetTrackOpData.cleanerMacAddress,STOP_CLEANER,(char*)"5,2");
      cStart = 0;
    }
    break;
  case C_ERROR_STATE: 
   ...
   case PA_WAIT_FOR_TIMEOVER_RESUME:
    if(IsPauseDelayTimeOver() == true)
    {
      ...
      if(getMode == AUTO || curManualState == MAN_TRACKCHANGER)
      {
           ...
           else if(GetTrackChangerError() == TC_CLEANER_ERROR || GetTrackChangerError() == TC_COMMUNICATION_ERROR)
           {
                SetTrackChangerState(TC_CLEANER_OPERATION);
                SetCleanerFSMState(C_SEND_RESUME);
           }

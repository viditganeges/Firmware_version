...
//  eMotor2State mot2Dir = GetMotor2State();
  ...
  case MOTOR1_IDLE:
    ...
    else if(mot1Dir == MOTOR1_NEGATIVE)
    {
      robotCurDirection = NEGATIVE;
    }
    else
...
//    }
//#else
    if((GetOperationMode()==MANUAL)||(GetOperationMode()==AUTO)) //(GetOperationMode()==MANUAL) //&&(GetManualState()!=MAN_TRACKCHANGER)) //changed for manual track changer
     {
       MotorDriveOn();			  
     } 
    SetRobotState(ROBOT_RE_START);
...
//        }
//#else
         if((GetOperationMode()==MANUAL)||(GetOperationMode()==AUTO))//if(GetOperationMode()==MANUAL) //&&(GetManualState()!=MAN_TRACKCHANGER)) //changed for manual track changer
         {
            MotorDriveOff();				   
         }	 
        SetRobotState(ROBOT_COMPLETED);
...
//    
    ...
    {  
      ...
      SendCommandToCleaner(getCleanerMAC->cleanerMAC_64,STOP_CLEANER,(char*)"5,2");
      dirChangeFlag = true;
      Serial_Debug("\n Robot End"); 
      currentPwm1Duty = STOP_PWM_DUTY;
//      currentPwm2Duty = STOP_PWM_DUTY;

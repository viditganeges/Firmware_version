...
{  
    PWM1_ChannelsStart(PWM_CHANNEL_3_MASK);
...
//    uint32_t period_value = PWM1_ChannelPeriodGet(STEPPER_PWM_CHANNEL);
    PWM1_ChannelDutySet(STEPPER_PWM_CHANNEL,(PWM1_ChannelPeriodGet(STEPPER_PWM_CHANNEL)/100)*(100-StepperDutyCycle));
    StopPwm();
    StartPwm();
...
//  uint32_t period_value = PWM1_REGS->PWM_CH_NUM[3].PWM_CPRD;
  PWM1_ChannelDutySet(PWM_CHANNEL_1, 0);

...
#define CMD_AUTO_GET_VALUES                    0x2222 
...
extern void mppt_bms_calculation(void);

extern char mppt_receieve_buffer[70];
...
extern bool ParseHeaderData(uint8_t* tempBuff, uint16_t* headerBufferLength, uint16_t command );
...
void SOC_log_Data(char *Log_dataas)
{
    ...
    ParseHeaderData(&txBuff[dataCount1],&bufferLength,CMD_SOC_LOG); //201C
...
//    uint8_t tempCount = snprintf(&Log_Array[dataCount],200,"%s,%s,%s,%s,%s,%s,%d,%s,%s,%s,%s,%s,%s,%s,%s,%s",Vin_str,Iin_str,Vbat_str, Ibat_str,Dtemp_str,Soc_Str,
//                                 GetChargingOrNot(),bat_arr,pv_arr,mpptcurr_arr,mpptout_arr,inttemp_arr,exttemp_arr,batstate_arr,loadout_arr,loadtrip_arr);        //Amberoot
//    mppt_bms_calculation();
    
//        uint8_t tempCount = snprintf(&txBuff[dataCount],bufferLength,"%s,%s,%s,%s,%s,%s,%d,%0.2f,%0.2f,%0.2f,%0.2f,%d,%0.2f,%0.2f,%d,%d,%d,%d,%d",Vin_str,
//                                 Iin_str,Vbat_str, Ibat_str,Dtemp_str,Soc_Str,GetChargingOrNot(),pv_vol,pv_curr,pv_power,battery_volt,batt_temp,batt_charg_curr,batt_discharg_curr,pv_work_status,Batt_SOC,charge_cont_temp,generated_energy,controller_status);
    
    
    snprintf(&Log_Array[dataCount],70,"%s,%s,%s,%s,%d,%s,%s,%d,%d,%d,%d,%d",pv_vol_arr,pv_curr_arr,pv_power_arr,battery_volt_arr,batt_temp,batt_charg_curr_arr,batt_discharg_curr_arr,pv_work_status,Batt_SOC,charge_cont_temp,generated_energy,controller_status);  
...
//    Serial_Debug("\r\nEnd");
//    UartTransmit(txBuff,dataCount1);
    stInterval* getInterval = GetSetIntervalLimits(); 
    if(getInterval->I6 == 1)
    {
...
void SOClog_main(void )
{
  ...
  if(SOC_SDcurntcount >= getInterval->I2 * 59500)//1000- 1sec    //59500 old
  {    
      SOC_logtime = true;      
  }
...
void MOTORCRNT_log_Data(char *Log_dataas)
{
    ...
    txBuff[dataCount1]=0;
    uint16_t bufferLength = sizeof(txBuff)-dataCount1;
    ParseHeaderData(&txBuff[dataCount1],&bufferLength,CMD_MOTORCURRENT_LOG); //2011
    dataCount1 = dataCount1+bufferLength;
    bufferLength = sizeof(txBuff)-dataCount1;
...
//    
    ...
    snprintf(&Log_Array[dataCount],100,"%d,%d,%s,%s,%s,%d,%d",mode_now,robotState,Imot1_Str,Imot2_Str,Imot3_Str,GetRotateSense1Count(),Currentmotordistance);
...
void ERROR_log_Data(char *Log_dataas)
{
    ...
    txBuff[dataCount1]=0;
    uint16_t bufferLength = sizeof(txBuff)-dataCount1;
    ParseHeaderData(&txBuff[dataCount1],&bufferLength,CMD_ERROR_LOG); //2018
    dataCount1 = dataCount1+bufferLength;
    bufferLength = sizeof(txBuff)-dataCount1;
...
//    uint8_t tempCount;
    ...
    snprintf(&Log_Array[dataCount],100,"%d,%d,%d,%d,%d,%s,%d",modeValue,robotState,mode_now,GetRotateSense1Count(),Currentdistance,SDerrorlog,Card_reset);
...
void AutoCycle_log_Data(char *Log_dataas)
{
    ...
    txBuff[dataCount1]=0;
    uint16_t bufferLength = sizeof(txBuff)-dataCount1;
    ParseHeaderData(&txBuff[dataCount1],&bufferLength,CMD_AUTO_CYCLE_LOG); //200F
    dataCount1 = dataCount1+bufferLength;
    bufferLength = sizeof(txBuff)-dataCount1;

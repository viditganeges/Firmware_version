...
//  uint8_t tempCount = snprintf(&txBuff[dataCount],bufferLength,"%d,%d,%000X,%d,%d,%s,%d",modeValue,mode_now,GetMotorFaultCode(),Currentdistance,robotState,cyclelog,Cumulative_distance);
  ...
  uint8_t tempCount = snprintf(&txBuff[dataCount],bufferLength,"%d,%d,%000X,%d,%d,%s,%s,%s,%s,%s,%d",modeValue,mode_now,GetMotorFaultCode(),Currentdistance,robotState,Cstarttime,Cstartsoc,Cendtime,Cendsoc,Cerrorcount,Cumulative_distance);
...
//                               GetRotateSenseCount(), GetNextIndex(),GetMotorFaultCode(), IsErrorFromCleaner(),bat_arr,pv_arr,mpptcurr_arr,mpptout_arr,inttemp_arr,exttemp_arr,batstate_arr,loadout_arr,loadtrip_arr);
  ...
  uint8_t tempCount = snprintf(&txBuff[dataCount],bufferLength,"%s,%s,%s,%s,%d,%s,%s,%d,%d,%d,%d,%d",pv_vol_arr,pv_curr_arr,pv_power_arr,battery_volt_arr,batt_temp,batt_charg_curr_arr,batt_discharg_curr_arr,pv_work_status,Batt_SOC,charge_cont_temp,generated_energy,controller_status);
  dataCount = dataCount+tempCount; 
  txBuff[dataCount] = STOP_CHAR;
  dataCount++;
  Serial_Debug("\r\ndataCount->");

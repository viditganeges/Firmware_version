...
extern char mppt_received_data[100];
int mpptflag =0;
...
{
  if((GetSwitchState()==MANUAL_CONTROL)||(GetSwitchState() == AUTO_CONTROL ))     //30.6.2022
  {  
...
static void CombineConfigUartReceiveData (uint8_t rxData)
{
  ...
  else if(GetSwitchState() == AUTO_CONTROL)
  {
    int i = 0;
    configUartRx.rxStartFlag = SET;
    if(configUartRx.rxBufferLength == 28)                     
    {
      configUartRx.Rxdata_Count_End = configUartRx.rxBufferLength;
      
      memcpy(mppt_received_data, configUartRx.rxBuffer,configUartRx.rxBufferLength);
      memset(configUartRx.rxBuffer,NULL,sizeof(configUartRx.rxBuffer));      
      configUartRx.rxCompleteFlag = SET;
      configUartRx.rxBufferLength = 0;
      configUartRx.rxStartFlag = RESET;
      mpptflag = 1;

...
uint32_t Cumulative_distance_cont = 0;
...
static eJsonStatus ProcessAcknowledgement (uint64_t macAddress, const char * jsonString)
{
  ...
  SetIsAckReceived();
  error_code = (eJsonStatus)atoi(jsonString);
  return error_code;
}
...
static eJsonStatus ProcessStatusResponse (uint64_t macAddress, const char * jsonString)
{
  ...
  uint8_t totalResultStringCount =  SplitString (jsonString, statusResponseStr, ',',maxNoStrings);
  ...
  if((totalResultStringCount == JSON_NO_OF_STATUS_RESPONSE))
  {
    ...
    if(cleanerStatus == CYCLE_COMPLETED)         /*ROBOT_COMPLETED*/
    {
...
{
  uint8_t txBuff[MAX_UART_TX_BUFFER_LENGTH];
  //eJsonStatus error_code = JSON_NO_ERROR;
  ...
  uint8_t tempCount = snprintf(&txBuff[dataCount],bufferLength,"%s",values);
  ...
  {
    return (ZigbeeApiTxData(macAddress,txBuff,dataCount));
  }
  else
  {
    return ERROR;
...
static eJsonStatus ProcessResponseCommand (stMessage message) 
{
  ...
  {
    jsonStatus = presentResponseCommandParams->executeCommand(message.deviceID, message.values);
    
//    if(mode == 2)
//    { 
//      MODE1 = message.values[0];
//      mode =0; 
//    }
  }
